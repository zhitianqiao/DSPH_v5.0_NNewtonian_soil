...
void JSphCpu::AllocCpuMemoryParticles(unsigned np,float over){
  ...
  if(InOut){
    //ArraysCpu->AddArrayCount(JArraysCpu::SIZE_4B,1);  //-InOutPartAcec
...
void JSphCpu::ComputeVerletVarsFluid(bool shift,const tfloat3 *indirvel
  ,const tfloat4 *velrhop1,const tfloat4 *velrhop2, const tsymatrix3f* sigma1, tsymatrix3f* sigma2, double dt,double dt2
  ,tdouble3 *pos,unsigned *dcell,typecode *code,tfloat4 *velrhopnew, tsymatrix3f* sigmanew)const
{
...
void JSphCpu::ComputeVerlet(double dt){
  ...
  if(VerletStep<VerletSteps){
    ...
    ComputeVerletVarsFluid(shift,indirvel,Velrhopc,VelrhopM1c,Sigmac,SigmaM1c,dt,twodt,Posc,Dcellc,Codec,VelrhopM1c,SigmaM1c);
    ComputeVelrhopBound(VelrhopM1c,twodt,VelrhopM1c);
  }
  else{
    ComputeVerletVarsFluid(shift,indirvel,Velrhopc,Velrhopc, Sigmac,Sigmac,dt,dt,Posc,Dcellc,Codec,VelrhopM1c, SigmaM1c);
    ComputeVelrhopBound(Velrhopc,dt,VelrhopM1c);
    VerletStep=0;
  }
  //-New values are calculated in VelrhopM1c. | Los nuevos valores se calculan en VelrhopM1c.
