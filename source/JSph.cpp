...
void JSph::LoadConfigParameters(const JXml *xml){
  ...
  switch(eparms.GetValueInt("RheologyTreatment",true,1)){
	...
	//-PressureTreatment switch
	switch (eparms.GetValueInt("PressureTreatment", true, 1)) {
	case 1:  Soil = false;    break;
	case 2:  Soil = true;     break;
	default: Run_Exceptioon("Pressure treatment is not valid.");
	}
...
void JSph::InitMultiPhase(const JXml *sxml,std::string xmlpath) {
  ...
  for(unsigned c=0; ele; c++) {
	...
	PhaseCte[c].E = sxml->ReadElementFloat(ele, "E", "value");
